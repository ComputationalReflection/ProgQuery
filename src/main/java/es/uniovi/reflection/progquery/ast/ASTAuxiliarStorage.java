...
public class ASTAuxiliarStorage {
    ...
    private Stack<List<Pair<NodeWrapper, List<MethodSymbol>>>> lastInvocationInStatementLists =
    ...
    public void checkIfTrustableInvocation(MethodInvocationTree methodInvocationTree, MethodSymbol methodSymbol,
        ...
        if (methodInvocationTree.getMethodSelect().getKind() == Kind.IDENTIFIER) {
            ...
            ExpressionTree memberSelectionExp =
    ...
    public void addInfo(MethodTree methodTree, NodeWrapper methodNode, MethodState methodState, int varArgParamIndex) {
        ...
        methodInfo.put(methodNode, new MethodInfo(methodTree, methodNode, methodState.identificationForLeftAssignExprs,
    ...
    public Map<TryTree, Map<Type, List<PartialRelation<CFGRelationTypes>>>> getTrysToExceptionalPartialRelations(
        ...
        for (Entry<TryTree, List<Pair<NodeWrapper, List<MethodSymbol>>>> entry : invocationsInStatements.entrySet()) {
            Map<Type, List<PartialRelation<CFGRelationTypes>>> typesToRelations =
                    new HashMap<Type, List<PartialRelation<CFGRelationTypes>>>();
    ...
    public void doInterproceduralPDGAnalysis() {
        Map<NodeWrapper, Iterable<RelationshipWrapper>> methodDecToCalls =
                new HashMap<NodeWrapper, Iterable<RelationshipWrapper>>();
        ...
        methodInfo.values().forEach(mInfo -> {
            ...
            Iterable<RelationshipWrapper> callRels =
                    mInfo.methodNode.getRelationships(Direction.OUTGOING, CGRelationTypes.CALLS);
        ...
        InterproceduralPDG pdgAnalysis =
                new InterproceduralPDG(methodDecToCalls, getDecs.getInvocationsMayModifyVars(), methodInfo);
    ...
    private void addCallToCallCache(RelationshipWrapper callRel) {
        ...
        for (RelationshipWrapper r : callRel.getEndNode()
